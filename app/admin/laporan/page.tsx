...
import { FiDollarSign, FiTruck, FiFileText, FiCheckCircle, FiAlertCircle, FiXCircle, FiLoader, FiDownload } from 'react-icons/fi';
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Calculate totals
        const revenue = checkouts.reduce((sum: number, item: any) => sum + (item.total_amount || 0), 0);
        const shipping = checkouts.reduce((sum: number, item: any) => sum + (item.shipping_cost || 0), 0);
    ...
    <div className="min-h-screen max-w-6xl ml-64 mx-auto py-10 px-4 bg-gradient-to-br from-pink-50 via-yellow-50 to-blue-50">
      ...
      {showExportTable && (
        <div ref={tableRef} className="bg-white rounded-2xl shadow-xl p-6 mb-8">
          <h2 className="text-xl font-bold mb-4 text-pink-700">Laporan Keuangan</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        <div className="bg-white rounded-2xl p-6 shadow-xl flex items-center gap-4 border-b-4 border-pink-200">
          ...
          <div>
            ...
            <p className="text-3xl font-extrabold text-pink-700">{data.length}</p>
          </div>
        </div>
