...
export default function KaryawanSidebar() {
  ...
  useEffect(() => {
    const activeIndex = menu.findIndex(
      item => {
        const link = `/karyawan/${item.name.toLowerCase()}`;
  ...
  return (
    <aside className="w-72 bg-gradient-to-br from-pink-100 via-pink-200 to-yellow-100 border-r p-7 space-y-8 fixed min-h-screen shadow-2xl flex flex-col z-50">
      <h1 className="text-3xl font-extrabold text-tomat tracking-wider mb-6 select-none drop-shadow flex items-center gap-2">
        ...
        <span className="ml-1">DMIN</span>
        <span className="ml-auto text-xs bg-white/60 px-3 py-1 rounded-full text-tomat font-bold tracking-widest shadow">KARYAWAN</span>
      </h1>
