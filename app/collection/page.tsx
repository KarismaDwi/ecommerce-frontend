    ...
    const fetchFlowerProducts = async () => {
      try {
        ...
        const response = await fetch(
          `http://localhost:7000/api/search?keyword=papan`
        );
        const data = await response.json();

        // Filter to ensure only 'papan' category is shown
        const flowerProducts = Array.isArray(data)
          ? data.filter((product: Product) =>
              product.kategori?.toLowerCase().includes("papan") ||
              product.name?.toLowerCase().includes("papan")
            )
          : [];
    ...
    <div className="relative mx-auto mt-32 p-6 bg-gradient-to-br from-blue-50 via-pink-50 to-yellow-50 min-h-screen">
      ...
      {products.length === 0 ? (
        <div className="text-center py-16">
          <p className="text-lg text-gray-500">Tidak ada produk papan ditemukan</p>
        </div>
      ) : (
